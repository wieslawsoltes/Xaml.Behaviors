...
public class ScreensCanvas : Control
{
    private double _leftMost;
    private double _topMost;
    private readonly IBrush _primaryBrush = SolidColorBrush.Parse("#FF0078D7");
    private readonly IBrush _defaultBrush = Brushes.LightGray;
    private readonly IPen _activePen = new Pen(Brushes.Black);
    private readonly IPen _defaultPen = new Pen(Brushes.DarkGray);
    ...
    public override void Render(DrawingContext context)
    {
        ...
        for (int i = 0; i < screens.ScreenCount; i++)
        {
            ...
            Rect boundsRect = new Rect(screen.Bounds.X / 10f + Math.Abs(_leftMost),
                screen.Bounds.Y / 10f + Math.Abs(_topMost),
                screen.Bounds.Width / 10f,
                screen.Bounds.Height / 10f);
            Rect workingAreaRect = new Rect(screen.WorkingArea.X / 10f + Math.Abs(_leftMost),
                screen.WorkingArea.Y / 10f + Math.Abs(_topMost),
                screen.WorkingArea.Width / 10f,
        ...
        if (topLevel is Window w)
        {
            ...
            context.DrawRectangle(_activePen,
                new Rect(wPos.X / 10f + Math.Abs(_leftMost), wPos.Y / 10f + Math.Abs(_topMost),
    ...
    {
        return new FormattedText(textToFormat, CultureInfo.CurrentCulture, FlowDirection.LeftToRight,
            Typeface.Default, 20, primary ? Brushes.White : Brushes.Black);
    }
}
