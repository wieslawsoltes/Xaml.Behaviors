...
namespace Xaml.Behaviors.SourceGenerators
{
    public partial class XamlBehaviorsGenerator
    {
        ...
        private void ExecuteGenerateAsyncTrigger(SourceProductionContext spc, AsyncTriggerInfo info)
        {
            ...
            sb.AppendLine("            {");
            sb.AppendLine($"                TrackTask({info.PropertyName});");
            sb.AppendLine("            }");
            sb.AppendLine("        }");
            sb.AppendLine();
            ...
            sb.AppendLine("            {");
            sb.AppendLine($"                TrackTask(change.GetNewValue<{info.PropertyTypeName}>());");
            sb.AppendLine("            }");
            sb.AppendLine("        }");
            sb.AppendLine();
            sb.AppendLine("        private void TrackTask(object? taskObj)");
            sb.AppendLine("        {");
            sb.AppendLine("            if (taskObj is null) return;");
            if (info.ResultTypeName != null)
            {
                sb.AppendLine($"            if (taskObj is Task<{info.ResultTypeName}> tt) {{ TrackTyped(tt); return; }}");
                sb.AppendLine($"            if (taskObj is System.Threading.Tasks.ValueTask<{info.ResultTypeName}> vtt) {{ TrackTyped(vtt.AsTask()); return; }}");
            }
            sb.AppendLine("            if (taskObj is Task t) { TrackVoid(t); return; }");
            sb.AppendLine("            if (taskObj is System.Threading.Tasks.ValueTask vt) TrackVoid(vt.AsTask());");
            sb.AppendLine("        }");
            sb.AppendLine();
            sb.AppendLine("        private void TrackVoid(Task task)");
            sb.AppendLine("        {");
            ...
            {
                sb.AppendLine();
                sb.AppendLine($"        private void TrackTyped(Task<{info.ResultTypeName}> task)");
                sb.AppendLine("        {");
                sb.AppendLine("            IsExecuting = true;");
                sb.AppendLine("            LastError = null;");
            sb.AppendLine("            task.ContinueWith(t =>");
        ...
        private void ExecuteGenerateObservableTrigger(SourceProductionContext spc, ObservableTriggerInfo info)
        {
            ...
            sb.AppendLine("            _subscription?.Dispose();");
            sb.AppendLine("            _subscription = null;");
            sb.AppendLine($"            var obs = {info.PropertyName};");
